...
def vime_semi(x_train, y_train, x_unlab, x_test, parameters, p_m, K, beta, file_name):
    ...
    def predictor(x_input):
        ...
        with tf.variable_scope("predictor", reuse=tf.AUTO_REUSE):
            ...
            inter_layer = contrib_layers.fully_connected(
                x_input, hidden_dim, activation_fn=act_fn
            )
            inter_layer = contrib_layers.fully_connected(
                inter_layer, hidden_dim, activation_fn=act_fn
            )

            y_hat_logit = contrib_layers.fully_connected(
                inter_layer, label_dim, activation_fn=None
            )
    ...
    for it in range(iterations):
        ...
        # Train the model
        _, y_loss_curr = sess.run(
            [solver, y_loss],
            feed_dict={x_input: x_batch, y_input: y_batch, xu_input: xu_batch},
        )
        # Current validation loss
        yv_loss_curr = sess.run(y_loss, feed_dict={x_input: x_valid, y_input: y_valid})
