...
def vime_main(label_data_rate, model_sets, label_no, p_m, alpha, K, beta):
    ...
    for m_it in range(len(model_sets)):
        ...
        results[m_it] = supervised_model_training(
            x_train, y_train, x_test, y_test, model_name, metric
        )
    ...
    y_test_hat = vime_semi(
        x_train, y_train, x_unlab, x_test, vime_semi_parameters, p_m, K, beta, file_name
    )
...
def exp_main(args):
    ...
    #%% Print results
    print(
        "Supervised Performance, Model Name: "
        + args.model_name
        + ", Avg Perf: "
        + str(np.round(np.mean(results[:, 0]), 4))
        + ", Std Perf: "
        + str(np.round(np.std(results[:, 0]), 4))
    )

    print(
        "VIME-Self Performance"
        + ", Avg Perf: "
        + str(np.round(np.mean(results[:, 1]), 4))
        + ", Std Perf: "
        + str(np.round(np.std(results[:, 1]), 4))
    )

    print(
        "VIME Performance"
        + ", Avg Perf: "
        + str(np.round(np.mean(results[:, 2]), 4))
        + ", Std Perf: "
        + str(np.round(np.std(results[:, 2]), 4))
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--iterations", help="number of experiments iterations", default=10, type=int
    )
    parser.add_argument(
        "--model_name", choices=["logit", "xgboost", "mlp"], default="xgboost", type=str
    )
    parser.add_argument(
        "--label_no", help="number of labeled data to be used", default=1000, type=int
    )
    parser.add_argument(
        "--p_m",
    ...
    parser.add_argument("--K", help="number of augmented samples", default=3, type=int)
    parser.add_argument(
        "--beta",
    ...
    parser.add_argument(
        "--label_data_rate", help="ratio of labeled data", default=0.1, type=float
    )
